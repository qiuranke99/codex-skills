...
INFERENCE_RULES: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"readable text|caption|text", re.I), "readable_text"),
...
PROPOSAL_MAP = {
    ...
    "readable_text": {
        ...
        "hypothesis": "Storyboard image prompts are allowing text-like marks that violate the no-readable-text constraint.",
        "rule": "Storyboard sheets must reject captions, labels, logos, subtitles, shot numbers, and readable text marks.",
        "applies_when": "Post-image QC detects readable text or text-like panel labels.",
        "risk": "Real packaging may contain identity marks; preserve form without legible text unless user explicitly requests it.",
        "regression": "Generated storyboard sheet with shot numbers or labels must be regenerated.",
    },
    "panel_separation_failure": {
        "target_file": "references/blue_gray_previs_style_bible.md",
