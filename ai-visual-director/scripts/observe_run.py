...
INFERENCE_RULES: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"product.*(label|text|logo|mark|packag|blank|fake|wrong)|label.*product|包装|标签|品牌文字|假品牌|空白瓶", re.I), "product_identity_mismatch"),
...
PROPOSAL_MAP = {
    ...
    "product_identity_mismatch": {
        "target_file": "SKILL.md",
        "hypothesis": "The product identity lock is missing or not being enforced strongly enough in storyboard planning/prompting.",
        "rule": "When a user product is visible, preserve the locked package silhouette, label layout, supplied product text, logo/mark, color blocks, and cap/pump/closure; reject blank packaging, fake brands, invented claims, and changed layouts.",
        "applies_when": "A shot plan, storyboard prompt, or generated sheet blanks, invents, or changes user-provided product packaging identity.",
        "risk": "Tiny microtext may be impossible in rough storyboard scale; preserve label geometry and primary supplied marks rather than inventing details.",
        "regression": "A user-provided bottle labeled LUMA / HYDRATING SERUM / 30 ml must not become a blank cosmetic bottle or a different fake brand.",
    },
    "panel_separation_failure": {
        "target_file": "references/blue_gray_previs_style_bible.md",
