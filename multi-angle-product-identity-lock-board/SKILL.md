...
`4k_enhancement_prompt_sha256`. Treat a returned external image as unverified
until its actual file is inspected against the accepted board and original
references. Verification requires preserved six-view geometry and seams, no
invented product detail, the complete source bundle, recorded actual dimensions
and ratio, and runtime evidence for both the requested 16:9 profile and 4K tier.
Missing evidence remains unverified; observed failures are rejected. A request
or provider label alone does not prove any of these checks passed.
...
An authorized project handoff may export
