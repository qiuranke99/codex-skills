...
def load_reference_manifest(manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("schema_version") != "multi_angle_reference_bundle.v1":
...
def validate_worker_rollout(
    ...
    if not agent_path.endswith(worker_run_nonce):
        raise ContractError(
            "blocked_worker_nonce_mismatch",
            "canonical worker agent path does not end with the exact run nonce",
        )
    ...
    if len(task_starts) != 1:
        raise ContractError(
            "blocked_worker_task_trace_missing",
            f"expected exactly one task_started event, found {len(task_starts)}",
        )
    worker_events = events[task_starts[0] :]
    ...
    post_image_custom_calls = [
        event
        for index, event in enumerate(worker_events)
        if index > image_end_index
        and event.get("type") == "response_item"
        and event.get("payload", {}).get("type") == "custom_tool_call"
    ]
    if post_image_custom_calls:
        raise ContractError(
            "blocked_worker_post_image_tool_call",
            "worker made a custom tool call after image generation completed",
        )
...
def main() -> int:
    ...
    if prompt_bytes.startswith(b"\xef\xbb\xbf") or b"\r" in prompt_bytes or prompt_bytes.endswith(b"\n"):
        raise ContractError(
            "blocked_generation_prompt_persistence",
            "frozen prompt sidecar must be UTF-8 without BOM, use LF internally, and have no terminal line break",
        )
    try:
