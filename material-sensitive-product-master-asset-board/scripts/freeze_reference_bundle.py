#!/usr/bin/env python3
"""Freeze ordered material-product references into one hash-locked run bundle."""
...
def parse_reference(value: str) -> tuple[str, Path]:
    ...
    if not ALIAS_RE.fullmatch(alias):
        raise BundleError(
            ...
            f"invalid reference alias {alias!r}",
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        required=True,
        help="Ordered alias=path entry; repeat once per unique source file",
    )
    return parser
...
def main() -> int:
    ...
    sources = [os.path.normcase(os.path.abspath(path)) for _, path in parsed]
    ...
    if len(set(sources)) != len(sources):
        raise BundleError(
            ...
            "list each unique source file once and express multiple roles in the source ledger",
        )

    reference_dir = run_dir / "sources" / "references"
    ...
    for index, (alias, source) in enumerate(parsed, 1):
        ...
        if sha256_bytes(frozen_bytes) != source_sha:
            raise BundleError(
