...
Use these fixtures for static or forward validation. Passing requires the exact expected state; attractive material rendering cannot override a state failure.

## Built-in dimensions are nonblocking

1. A built-in board returns `1672x941`, while hero, complementary views, material boundaries, structure, and panel legibility pass.
...
## Integrity, material, and external 4K

1. Mutate one byte in either prompt sidecar. Expect `blocked_prompt_pair_integrity`; forbid reconstruction.
2. Omit the original material/product references or Codex board from the external bundle. Expect handoff not ready.
...
4. Return exact 4K/16:9 pixels after smoothing away a liquid boundary, glass thickness, refraction edge, microbubble, seam, or real highlight transition. Expect `no_over_denoising: fail`, external QA `failed`, and `external_4k_status: rejected`.
5. Invent an inner tube, facet, fill level, reflection, or hidden structure. Expect external QA `failed` and `external_4k_status: rejected`.
