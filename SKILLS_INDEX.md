...
| `complex-structure-product-asset-lock-board` | `D:\AI\skill\complex-structure-product-asset-lock-board` | Create separate video-ready complex structure product asset boards that lock structure overview, critical closeups, state variants, interface/operation areas, decomposition relationships, materials, and high-risk nodes using Codex built-in `/image gen`, final image-generation prompt output, and QA. | `D:\AI视觉工作室\.agents\skills\complex-structure-product-asset-lock-board` | Active |
