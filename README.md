...
公开维护的个人 Codex Skill 仓库，以及一套完整的 **High-Control AI TVC
Production System（高控制全能参考 AI TVC 生产系统）**。
...
所有 15 个 Skill 在仓库根目录各保留一个唯一目录。核心 Skill 具有
sibling dependency，不能把同一套 Skill 再复制到子系统目录，也不能
把它们拆散到不同父目录。
...
安装生命周期、PowerShell/POSIX 语法和 13-package 合同面。真实工作站
