...
公开维护的个人 Codex Skill 仓库。仓库根目录的 18 个 Skill 都是独立
...
排除边界和唯一 discovery。它不安装、更新或签署其余 17 个包。
...
上述 18 个 Skill 在仓库根目录各保留一个唯一包目录。任何包的核心能力都
...
先运行与 High-Control 无关的 18 包独立性验证：

```bash
python .github/scripts/validate_standalone_skills.py --repo-root . --expected-count 18 --timeout 180
```
...
18 包独立性、可选 15 包聚合兼容性、安装生命周期和 PowerShell/POSIX
语法。真实工作站
