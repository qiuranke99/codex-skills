...
Use for maintenance or relevant failure investigation. Canon cases apply only
to explicitly requested integration, not every standalone Look package.
...
8. **First-class visual references**: every internal State reference maps to a distinct nested artifact, separately hash-verified primary bytes and complete matching owned-record sidecar. Explicit integration additionally verifies active Canon entries and the root-plus-references receipt. Pass.
...
8. **Uninspected reference**: a generated image is marked approved before its actual completed pixels are inspected. Fail regardless of turn number; completed generation and inspection may share a turn.
...
15. **Tampered visual reference**: its scope differs from assigned States, primary bytes drift, or its local owned sidecar is missing, malformed or differs from the complete nested hash-bound record. Fail without requiring Canon. In requested integration, missing Canon/receipt registration also fails.
