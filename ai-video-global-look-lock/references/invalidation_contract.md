...
`changed_json_pointers` must exactly equal the real semantic pre-to-post diff. The validator derives `look_core_changed`, changed State IDs and propagated Shot UIDs from the actual stable-ID records; a self-reported scope cannot hide an extra State, reference or Shot change. Invalidated and preserved artifact IDs are disjoint. Standalone downstream preservation claims require supplied immutable artifact/file locks; without them, report downstream status as unverified instead of inventing registry evidence. Only for explicitly requested Project Canon integration must each preservation claim also pass the transition validator with byte-identical base and post Canon entries.
