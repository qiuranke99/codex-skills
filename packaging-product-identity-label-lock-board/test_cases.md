...
Use these fixtures for static or forward validation. Passing requires the exact expected state; packaging-copy evidence and visual plausibility cannot override a state failure.

## Built-in dimensions are nonblocking

1. A built-in board returns `1672x941`, while package geometry, views, and label layout pass.
   - Expect `built_in_dimensions_policy: evidence_only_nonblocking`.
   - Expect `codex_board_role: content_qa_reference` and geometry/layout QA `passed`.
...
## Integrity, exact copy, and external 4K

1. Mutate one byte in either prompt sidecar. Expect `blocked_prompt_pair_integrity`; forbid reconstruction.
2. A generative 4K result makes text look sharper but has no field-level OCR match, deterministic composite, logo-region proof, or barcode/QR decode. Expect `exact_copy_lock_status: conditional_unverified` or `not_approved`, never `approved`.
3. Omit the original product references, Codex board, or required exact-copy assets from the external bundle. Expect handoff not ready.
   - Even when both prompt hashes pass, `prompt_pair_ready` must not set `external_4k_status: handoff_ready`.
4. Omit exact external `aspect_ratio: "16:9"` or `image_size: "4K"` controls. Expect `blocked_runtime_controls`.
5. Return exact 4K/16:9 pixels but alter package geometry or label layout. Expect external QA `failed` and `external_4k_status: rejected`.
