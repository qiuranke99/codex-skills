...
description: "Create text-free casting-reference boards for one selected character: a large portrait and complete front, back, and side views, with extensions only for source-backed continuity risks. Include visual QA and each accepted board's generation/enhancement prompt pair."
---
...
7. Present the exact generation prompt, board ID, and hash. Set `prompt_disclosed_before_generation: true`; state that the inspected-board-specific 4K prompt will be delivered only after post-generation visual inspection with the original references available.
...
9. Call the available image tool, then inspect its completed result as soon as the current host permits.

Follow the current image-tool contract. When the completed board is available
and the host permits continuation, inspect it, finalize its prompt pair, and
continue the authorized board set in the same turn. A multi-board package is
ordered by dependencies, not by one-call-per-turn quotas: inspect or repair A
before generating B, C, or D. Do not require another user message for that work.

If the actual host ends the turn or the output is pending, preserve the exact
attempt and resume on the available continuation. Do not assume an untriggered
continuation or claim completion before every required board is inspected.

Retain existing state-field names for compatibility. Set
`terminal_generation_call: executed` from the completed tool result and count
each actual original or repair call exactly once in `generation_attempt_count`.
`generated_board_count` counts only unique accepted/current boards. The legacy
labels `generation_terminal_pending` and `awaiting_post_generation_continuation`
do not mandate ending a turn. Only actual inspection can finalize a 4K prompt.
...
- an optional `qa_report.md` based on actual post-generation inspection.
...
`terminal_generation_call` stays `pending` until the tool/runtime trace proves `executed` when the completed result is available. `assistant_qa_status` and `production_approval_status` are separate. Without independent post-generation visual review, assistant QA remains `pending_post_generation_inspection`; production approval remains `not_granted` until the user or an authorized external pipeline explicitly sets `user_granted` or `external_pipeline_granted`.

## 7. Post-Generation Visual QA

Inspect each returned board only when the completed result is available. For the main board, verify:
...
Use `passed`, `conditional`, or `failed`. Keep production approval unchanged. If one targeted repair is justified, freeze and disclose a new complete prompt and hash before a new image call, then inspect its actual result under the current tool contract.
...
If more boards remain planned, persist the verified pair and continue the dependency-ordered generation and inspection sequence. After every accepted board has a verified pair, require `generated_board_count == finalized_4k_prompt_count == 4k_prompt_hash_count == 4k_handoff_sidecar_count` and coverage of every `accepted_board_id`, then set `task_finalization_status: prompt_pair_ready`. `prompt_pair_ready` proves prompt integrity only and does not imply any board or package `external_4k_status: handoff_ready`; the external reference and runtime-control gates remain independent.
