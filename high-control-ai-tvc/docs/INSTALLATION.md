...
The 20 top-level Skills in this repository are independent packages. Each can
...
- Explicit aggregate `all` profile: 15 managed workflow members
  (13 core + 2 optional explorers); `excluded_from_aggregate_profile` keeps five
  standalone Skills outside aggregate installation and receipts only. All 20
