...
仓库中的 18 个顶层 Skill 仍然独立安装、发现、调用和验证；其 package-local
合同不以本 SOP、suite manifest、receipt、release gate、pinned runtime 或
兄弟包为前置条件。只调用某一个 Skill 时，直接遵守该包 `SKILL.md` 和本地
validator。

## Aggregate workflow release 门

当用户明确把项目登记为 aggregate-managed workflow 后，GitHub
`qiuranke99/codex-skills` 的 `main` 是该 profile 的跨机 snapshot 更新源。
新项目、恢复项目和每个 0–10 阶段开始前，运行当前不可变 snapshot 内的
OS-native `tools/release-control.ps1` / `tools/release-control.sh` 固定入口执行
`check` 并要求 `ready_latest=true`。

把返回的 `release_commit` 写入本阶段 aggregate runtime/dependency lock，
并在该原子阶段内冻结它。GitHub 更新、离线、验证失败、
receipt/snapshot/discovery 漂移或当前 Codex task 需要重启时，只阻塞这个
aggregate-managed 阶段：运行 `sync` 并从新 task 继续，不在同一 Canon
transaction 混用两个 revision。这些状态不改变任何单包的独立 availability。
...
| 2. Canon 资产锁定 | 7 个窄域资产 Owner | Codex Image Generation、原始参考图、Owner validator、aggregate Canon bridge | Shot Contract 的资产需求、角色/产品/包装/材质/场景参考 | 为每个资产选择唯一适用 Owner；生成、后验检查、批准；经显式 fixed profile bridge 写入 Canon | 角色、产品、包装、材质、场景 Canon 资产与记录 | 图片真实存在并经后验检查；Owner QA 通过；明确生产批准；Canon transition 通过 | 身份/几何/文字/材质证据冲突 → 回对应 Owner；复杂机制无专属 Owner → 仅阻塞该资产范围，不猜测结构 |
...
节点 2 的跨包写入是 aggregate-only 能力：使用
`<SYSTEM_ROOT>/tools/build_asset_canon_export.py asset --profile <固定七选一>`，
不得恢复 owner 包内的共享 bridge wrapper，也不得接受自由文本 owner。节点 1、3、
4、5、6、10 对既有 Canon 的 workflow transition，必须把 package-local
`apply_project_canon_transition.py` 的 `--transition-runner` 绑定到
`<SYSTEM_ROOT>/tools/canon_runner_<owner_slug>.py` 六个固定 runner 之一。该共享
内核统一执行项目锁、pending journal 恢复/阻塞、raw-byte compare-and-swap、
持久 readback 和 applied receipt；普通结构 validator 或
`validate_ai_video_aggregate.py` 不能替代一次真实 transaction。
...
对 aggregate-managed 项目，还必须满足该项目记录的 release/receipt/runtime
lock；这只证明聚合工作流的版本一致性。单 Skill 是否可用仍由其自身包与
package-local validation 决定。
