...
- **独立 Skill（默认）**：直接调用该顶层包，读取其 `SKILL.md` 并运行
  package-local validator。不要要求 `SYSTEM_ROOT`、suite manifest、aggregate
  receipt、release gate、pinned runtime、同 release 或兄弟包。
- **High-Control aggregate workflow（显式 opt-in）**：使用下面的 Master、
  续跑或恢复 Prompt，即表示用户选择了批量维护、不可变 snapshot、aggregate
  preflight 与跨包版本锁。此时这些 gate 只约束该项目的聚合事务；失败不能
  被解释为某个单包不可用。

## 1. 开始前只替换这些变量

```text
<SYSTEM_ROOT>           仅 aggregate workflow：release check 返回的 active_system_root
<PROJECT_ROOT>          项目绝对路径
...
```text
<SYSTEM_ROOT> = <active_system_root returned by the aggregate launcher>
...
不要手工猜测 release-state 目录；以 launcher 的 machine-readable
`active_system_root` readback 为准。

路径中有空格时始终保留完整绝对路径，并让 Codex 在命令层正确加引号。Windows 旧 `.doc`/`.rtf` 需要先通过 Word 或受信任转换器保存为 `.docx`；不要让 Codex猜测无法读取的二进制文本。

## 2. 全流程 Master Prompt

把下面整段交给 Codex，即明确选择 aggregate-managed workflow。它会从粗脚本
开始，推进到每个真实审批门，而不是要求用户先写专业导演脚本。
...
1. 本 Master Prompt 已明确把项目选择为 aggregate-managed workflow。在写入本项目的 aggregate artifact 前，先运行 OS-native profile 入口：Windows 使用 `<SYSTEM_ROOT>\tools\release-control.ps1 -Action check -ProjectRoot <PROJECT_ROOT> -Format json`，macOS/Linux 使用 `<SYSTEM_ROOT>/tools/release-control.sh check --project-root <PROJECT_ROOT> --format json`。只有 `ready_latest=true` 才能继续该聚合事务，并把返回的 `release_commit` 写入本阶段 aggregate runtime/dependency lock；若远端更新、离线、snapshot/receipt/discovery 漂移或提示 `PROCESS_RESTART_REQUIRED`，停止该聚合阶段，用同一入口运行 `sync` 后打开新的 Codex task，不在同一 Canon transaction 混用两个 revision。随后完整读取 <SYSTEM_ROOT>/docs/SOP.md、<SYSTEM_ROOT>/docs/TOOLS_INPUTS_OUTPUTS.md、<SYSTEM_ROOT>/docs/REVISION_AND_APPROVAL.md、<SYSTEM_ROOT>/docs/PROJECT_STRUCTURE.md，以及当前阶段 Skill 的 SKILL.md 和其要求的 references。若 <SYSTEM_ROOT> 不存在或不含 SUITE_MANIFEST.json，报告 `aggregate_profile_blocker`；不要把它描述成任一单包不可安装、不可发现或不可调用。
...
12. 每阶段先生产真实 artifact，再运行该 Skill 的 validator 和 Project Canon transition。节点 2 资产注册使用 `<SYSTEM_ROOT>/tools/build_asset_canon_export.py asset --profile <固定七选一>`；六个 workflow Owner 的 package-local transition 必须传入 `<SYSTEM_ROOT>/tools/canon_runner_<owner_slug>.py` 对应固定 runner。validator 只证明结构与证据闭合，不代表用户审美批准、法律批准或 provider 一定服从。
...
16. 仅因本项目已选择 aggregate-managed workflow，每次从用户批准暂停恢复到下一阶段时重新运行同一个 profile gate。一个原子阶段内冻结同一 `release_commit`；GitHub `main` 变化时先停止并迁移，禁止同一 Canon transaction 混用两个 Skill revision。

开始时先：
- 检查 Skill 是否可发现、Python/Pillow/FFmpeg/ffprobe 是否可用；
- aggregate validator 使用 profile receipt `validation.python_executable` 对应的运行时，或由 `AI_TVC_PYTHON` 显式指定的兼容运行时；这不覆盖 package-local validator 自己声明的解释器合同；
- 检查源文件实际可读；
...
本项目已显式选择 aggregate-managed workflow。请先运行 OS-native profile 入口（Windows：<SYSTEM_ROOT>\tools\release-control.ps1 -Action check -ProjectRoot <PROJECT_ROOT> -Format json；macOS/Linux：<SYSTEM_ROOT>/tools/release-control.sh check --project-root <PROJECT_ROOT> --format json），并要求 ready_latest=true；若更新则先用同一入口 sync 并打开新 task。这个状态只约束本项目的 aggregate transaction，不是单包 availability 判断。然后把批准证据写入对应 Owner 合同并验证 Canon transition，读取 <PROJECT_ROOT> 的完整 Project Canon，确认没有 stale、blocked、hash drift 或未完成 transaction。按照 <SYSTEM_ROOT>/docs/SOP.md 从当前阶段继续到下一个真实用户批准门。不要跳阶段，不要重新生成未受影响资产，不要调用第三方视频 API。
```
...
以下单阶段 Prompt 可以独立使用。只有当当前项目已明确登记为
aggregate-managed workflow 时，才在它们之前增加 aggregate release check；
独立调用时直接遵守目标包 `SKILL.md` 与 package-local validator。
...
不得为同一 asset_key 保留两个终端角色 Authority。按各 Owner 原始视觉工作流生成并在下一回合检查真实图像；只有显式生产批准后，才用 `<SYSTEM_ROOT>/tools/build_asset_canon_export.py asset --profile <上面唯一 Owner 对应 profile>` 注册。必须提供实际图片、prompt、approval evidence 的 project-relative locator 与 SHA-256、受影响 Shot UID 和固定 authority mode；不得使用自由文本 owner，也不得从 owner 包定位共享 wrapper。随后用 `validate_asset_canon_export.py` 绑定实际 Canon entry 复核。不要让 Prompt Director 代替资产 Owner 导出 Canon。
```
...
```text
请恢复 <PROJECT_ROOT> 的 High-Control AI TVC aggregate-managed 工作流。SYSTEM_ROOT: <SYSTEM_ROOT>。该项目已显式选择 aggregate profile；先运行 profile release check 并要求 GitHub main 与本阶段 snapshot 一致；不一致时只停止该聚合事务，sync、迁移并打开新 task，不要据此宣称任一单包不可用。
...
若项目从未选择 aggregate profile，不使用上述恢复 Prompt；按实际参与的独立
Skill 合同和 artifact hashes 恢复，不补造 suite receipt 或
`SYSTEM_RUNTIME_LOCK.json`。
