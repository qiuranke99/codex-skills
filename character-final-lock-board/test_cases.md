...
3. **Runtime continuation**: inspect and finalize in the same task after the image returns when supported. If the actual host ends the turn, retain `task_finalization_status: awaiting_post_generation_continuation`; generation alone is only stage-complete.
4. **Inspection required**: inspect the actual returned board before creating `final_4k_enhancement_prompt`; never ask for a routine user "continue" when the host can proceed.
