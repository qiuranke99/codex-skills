...
| User approves a source-limited board whose QA remains conditional | Deliver it with limits; approval alone does not satisfy the separate passed-QA gate for terminal authority. |
| Board is visible but its bytes cannot be read, even with approval and two valid prompt hashes | Do not claim a persisted or hash-bound terminal board; keep authority promotion blocked. |
| The board file is replaced after its accepted attempt was inspected | Recomputed board hash must still bind to that accepted attempt; do not promote the replacement on old QA or approval. |
| Same asset already has a terminal casting owner | Do not add a competing simultaneous terminal owner. An explicit user reassignment may supersede the old route; other assets and non-terminal work remain unaffected. |
| External result is labeled 4K but has no evidence for the requested 16:9 profile | Record the returned evidence and keep it unverified; a provider label is not acceptance. |
| External output meets resolution controls but adds a face in the back-body panel | Reject the returned board for topology failure; resolution evidence cannot compensate. |
| External file, original references, requested profile evidence, identity and topology all pass actual inspection | External verification may pass; this alone does not grant terminal project authority or human production approval. |
