...
Treat a returned external image as unverified until its actual file is
inspected against the accepted board and original references. Record observed
dimensions and ratio, plus runtime evidence for the requested 16:9 profile
and 4K tier. Mark it verified only when those checks, source fidelity, identity,
and the exact single-face topology pass. Missing evidence remains unverified;
observed failures are rejected. A request or provider label alone is not
verification, and external enlargement never establishes native provenance.
...
Optional authority promotion requires `assistant_qa_status: passed`, readable
accepted-board bytes with a recomputed SHA-256 bound to the accepted attempt,
exact byte/hash readback of both accepted prompt sidecars, and explicit
production approval. A conditional or byte-unavailable board may be delivered
with its limits, but must not become terminal project authority.

An authorized project handoff may export the approved board and prompt paths/hashes,
...
`terminal_route_decision: single_face_character`. The external integrator must
select one current terminal character route for the same asset key; this route
cannot coexist with competing terminal casting or character-final owners.
The user may explicitly reassign that responsibility; preserve the superseded
route as history, not a simultaneous authority. This restriction does not block
other assets or non-terminal work. Do not write Project Canon or require an
external integrator to finish this board.
