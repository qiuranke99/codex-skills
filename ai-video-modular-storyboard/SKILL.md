...
Read `test_cases.md` when maintaining the Skill or investigating a regression; ordinary storyboard work does not require loading the test catalog.
...
Rebuild or promote every required frame only after character/product/scene assets and the Global Look version are bound. Preserve the approved composition while applying identity, geometry, material, packaging, scene, wardrobe, and look evidence. A frame is downstream-ready only after post-generation visual inspection and approval.
